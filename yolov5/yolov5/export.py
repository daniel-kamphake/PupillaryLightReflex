...
def export_torchscript_ptl(model, im, file, optimize, prefix=colorstr('TorchScript_PTL:')):
    ...
    try:
        ...
        # Change in this line 
        # according to https://pytorch.org/tutorials/prototype/lite_interpreter.html
        (optimize_for_mobile(ts) if optimize else ts)._save_for_lite_interpreter(f.name)
...
def run(data=ROOT / 'data/coco128.yaml',  # 'dataset.yaml path'
        ...
        include=('torchscript', 'torchscript_ptl', 'onnx', 'coreml'),  # include formats
    ...
    if 'torchscript_ptl' in include:
        export_torchscript_ptl(model, im, file, optimize)        
    if 'onnx' in include:
...
def parse_opt():
    ...
    parser.add_argument('--include', nargs='+',
                        ...
                        help='available formats are (torchscript, torchscript_ptl, onnx, coreml, saved_model, pb, tflite, tfjs)')
